...
export default function SettingsPanel({ settings, onUpdate, onClose }) {
  ...
  return (
    <div
      className="border border-border bg-surface animate-slide-up"
      ...
      <div className="p-4 space-y-5">
        ...
        {settings.aiMode === 'ollama' && (
          <div className="space-y-3 border-t border-border pt-4">
            ...
            {/* Ollama URL */}
            <div>
              <label className="text-dim text-xs block mb-1">ollama url</label>
              <div className="flex items-center border border-muted focus-within:border-accent transition-colors">
                ...
                <input
                  ...
                  className="flex-1 bg-transparent text-text text-xs py-2 pr-2 outline-none"
                  style={{ fontFamily: 'var(--font-mono)' }}
                />
              </div>
            </div>
            ...
            <div>
              ...
              {availableModels.length > 0 ? (
                ...
                <div>
                  <div className="flex items-center border border-muted focus-within:border-accent transition-colors mb-2">
                    <span className="text-accent text-xs px-2">›</span>
                    <input
                      type="text"
                      ...
                      className="flex-1 bg-transparent text-text text-xs py-2 pr-2 outline-none placeholder:text-muted"
                      style={{ fontFamily: 'var(--font-mono)' }}
                    />
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {OLLAMA_COMMON_MODELS.map((m) => (
                      <button
                        key={m}
                        onClick={() => onUpdate({ ollamaModel: m })}
                        className="text-xs px-2 py-0.5 border border-muted text-dim hover:text-text hover:border-accent transition-colors"
                      >
                        {m}
                      </button>
                    ))}
            ...
            {ollamaStatus === 'offline' && (
              <div className="border border-red-900 bg-red-950 bg-opacity-20 p-3 text-xs text-red-400 space-y-1">
        ...
        {settings.aiMode === 'openrouter' && (
          <div className="border border-muted p-3 text-xs text-dim space-y-1">
