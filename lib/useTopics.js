...
export function useTopics() {
  ...
  const reorderTopics = useCallback((activeId, overId) => {
    const oldIndex = topics.findIndex((item) => item.id === activeId)
    const newIndex = topics.findIndex((item) => item.id === overId)
    if (oldIndex !== -1 && newIndex !== -1 && oldIndex !== newIndex) {
      persistTopics(arrayMove(topics, oldIndex, newIndex))
    }
  }, [topics, persistTopics])
